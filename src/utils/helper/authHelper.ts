	...
	providers: [
		CredentialsProvider({
			...
			async authorize(cred) {
				...
				return {
					...
					role: account.isSuperAdmin ? "superadmin" : "admin",
		...
		CredentialsProvider({
			...
			credentials: {
				...
				isGuest: { label: "Guest Checkout", type: "text" },
			},
			async authorize(cred) {
				...
				if (cred.isGuest === "true") {
					const guestPhone = `guest_${cred.table}_${Date.now()}`;
					customer = await new Customers({ fname: "Guest", lname: "Customer", phone: guestPhone }).save();
				} else {
					...
					customer = await Customers.findOne({ phone: cred?.phone });
				...
				if (!account?.tables?.some?.((t: any) => t.username === cred?.table && t.isActive))
					throw new Error("This table is currently locked. Please contact staff.");
