	...
	return (
		<div className="orderDetailsPreview">
			...
			<div className="contentScroll">
				...
				{(order.staffCalls?.length ?? 0) > 0 && (
					<div className="section staffCallsList" style={{ marginTop: "-1rem" }}>
						<h3>Staff Calls</h3>
						<div className="table">
							<div className="row headerRow">
								<span className="col time" style={{ width: "25%", fontWeight: 600 }}>
									Time
								</span>
								<span className="col reason" style={{ flex: 1, fontWeight: 600 }}>
									Reason
								</span>
								<span className="col status" style={{ width: "20%", textAlign: "right", fontWeight: 600 }}>
									Status
								</span>
							</div>
							{order.staffCalls?.map((call, index) => (
								<div key={index} className="row">
									<span className="col time">{new Date(call.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
									<span className="col reason">{call.reason}</span>
									<span className="col status" style={{ textAlign: "right" }}>
										<span
											style={{
												...
												backgroundColor: call.status === "active" ? "#fff3f3" : "#f0fdf4",
												color: call.status === "active" ? "#e74c3c" : "#16a34a",
											}}>
											{call.status}
										</span>
									</span>
