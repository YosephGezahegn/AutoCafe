...
const MenuEditorItem = (props: TMenuEditorItemProps) => {
	const { item, onEdit, onHide, onDelete, hideSettingsLoading = false } = props;
		...
		<div className="menuEditorItem" ref={itemRef}>
			{inView && (
				<>
					...
					<div className="menuItemData" onClick={() => onEdit(item)} style={{ cursor: "pointer" }}>
						<h5 className="menuItemTitle">{item.name}</h5>
						<p className="menuItemDesc">{item.description}</p>
						<p className="menuItemPrice birr">{item.price}</p>
					</div>
					<div className="menuItemOptions">
						...
						<Button icon="f2ed" iconType="solid" size="mini" type="secondaryDanger" loading={hideSettingsLoading} onClick={() => onDelete(item)} />
					</div>
				</>
			)}
		</div>
