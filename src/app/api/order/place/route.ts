...
export async function POST(req: Request) {
	try {
		...
		const tableData = await Tables.findOne({ username: session?.restaurant?.table, restaurantID: session?.restaurant?.username });
		if (!tableData?.isActive) throw { status: 403, message: "Table is locked by staff. Cannot accept orders at this time." };
		...
		const newOrder = new Orders({ restaurantID, table, customer, sessionId: tableData.activeSessionId, products: products });
		await newOrder.save();
