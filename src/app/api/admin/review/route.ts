...
export async function GET() {
	try {
		...
		await connectDB();
		const reviews = await Reviews.find({ restaurantID: session.username }).sort({ createdAt: -1 }).lean();
...
export async function DELETE(req: Request) {
	try {
		...
		const review = await Reviews.findOne({ _id: reviewId, restaurantID: session.username });
