	...
	if (!props.selectedProducts.length && !order?.products?.length) {
		return (
			<div className="cartPage">
				<NoContent label={t("Aren't you hungry?")} animationName="FoodBurgerHappy" />
			</div>
		);
	}
			...
			<div className="cartPage">
				<div className="cartApproval">
					...
					<div className="approvalHeading">
						<p>{t("Your order")}</p>
						<p>{t("will be accepted soon")}</p>
					</div>
					<Button className="endOrder" type="secondaryDanger" size="mini" label={t("Cancel Order")} loading={cancelingOrder} onClick={onCancelOrder} />
				</div>
			</div>
		);
	}
			...
			<div className={`cartCheckout ${bottomBarActive ? "active" : ""}`}>
				<div className="checkoutHeader">
					{approvedProducts && (
						<div
							...
							{bottomBarActive ? (
								<h5>
									{t("Bill")} <span>{t("Summary")}</span>
								</h5>
							) : (
								<>
									<p>{t("Sub Total")}</p>
									<span className="totalValue birr">{order?.orderTotal} </span>
									{order?.orderTotal && (
										<span className="plusTaxes">
											{" "}
											+ Br {order?.taxTotal} {t("Tax")}
										</span>
									)}
								</>
							)}
				...
				{order && (
					<div className={clsx("taxDetails", showTaxSummary && "show")}>
						<CartTaxItem name={t("Item Total")} amount={order?.orderTotal} />
						<hr className="itemHr" />
						<CartTaxItem
							className="taxSummaryTitle"
							name={showTaxSummary ? t("Tax Summary") : t("Tax Total")}
							subtitle={showTaxSummary ? t("close") : t("show details")}
						...
						<hr />
						<CartTaxItem name={t("Grand Total")} amount={order?.orderTotal + order?.taxTotal} />
					</div>
				)}
			</div>
		</div>
