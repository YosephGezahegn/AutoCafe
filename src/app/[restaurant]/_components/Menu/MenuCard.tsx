...
const MenuCard = (props: TMenuCardProps) => {
	...
	const handleCardClick = (e: any) => {
		if (e.target.closest(".quantityButton") || e.target.closest(".info")) return;
		setShowInfo(true);
	};

	return (
		<div id={`menu-item-${item._id}`} className={classList + (!inView ? "blank" : "")} ref={cardRef} onClick={handleCardClick}>
			{inView && (
				<>
					{item.image && (
					...
					<div className="options">
						...
						<div className="footer">
							{!item.image && <div className="priceNoImage birr">{item.price}</div>}
							<QuantityButton
								className="addToCart"
								quantity={quantity}
								filled
